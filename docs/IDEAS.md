...
- [x] Wall of Hearts — `/wall`: live gallery of recently beating cards.
      Opt-in only (`?wall=1` on your own embed → `pulse:wall` set); the
      anonymous counter still counts every render via `pulse:hearts`.
      Rendering someone else's card never publishes them
